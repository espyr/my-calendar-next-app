...
import { getTodos, addTodo, updateTodo, deleteTodo } from "@/apiCalls/todosApiCalls";
...
const MyTodos: React.FC = () => {
  ...
  const handleAddTodo = async () => {
    const newTodo = await addTodo({ description: newTodoDescription, completed: false });
    // setTodos([...todos, newTodo]);
    setNewTodoDescription("");
  };

  const handleToggleTodo = async (id: string, completed: boolean) => {
    const updatedTodo = await updateTodo({ id, completed: !completed, description: "" });
